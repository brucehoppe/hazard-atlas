...
Earthquakes retain the inherited USGS GeoJSON schema. Wildfire incidents use namespaced `eonet:<id>` or `cwfis-active:<id>` source identifiers, title, description, provider status, source links, and an ordered list of dated Point or Polygon geometries. CWFIS agency-reported incidents additionally retain agency, area in hectares, percent contained, control status, and status-update time as provider fields. FIRMS records use a product-specific `firms:<sha256>` identity derived from product, satellite, acquisition minute, coordinates, and version. Repeated retrievals of the same observation deduplicate without combining separate overpasses.

Temporal precision is explicit. EONET date-only geometry is stored as a day, not an invented midnight event. CWFIS active-fire status updates and FIRMS acquisition times are UTC timestamps with provider precision. Provider status, agency control status, detector confidence, and earthquake review status are distinct fields. Unknown values remain null or unavailable; zero FRP is retained as numeric zero.
