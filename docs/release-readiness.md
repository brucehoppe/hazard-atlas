...
| Attribution/license | The production footer carries the plain-text credit `Built by Bruce Hoppe`, with no repository link and no address published in the interface; browser check asserts its visibility. The About and Sources panels still link the source code, licence and issue tracker. README, QUICKSTART, MIT LICENSE and third-party notices included in archives. |
