...
async function checkDisplayedInk(page, dpr) {
  await page.evaluate(async () => {
    const font = await new FontFace(
      "Country Labels",
      `url(${window.labelFontUrl})`,
    ).load();
    document.fonts.add(font);
    const source = document.querySelector('[data-country="Canada"]');
    const panel = document.createElement("div");
    panel.style.cssText =
      "position:fixed;left:0;top:0;width:180px;height:90px;background:white;z-index:99999";
    ...
    const hinted = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "text",
    );
    hinted.textContent = source.dataset.country;
    for (const [name, value] of Object.entries({
      "font-family": "Country Labels",
    ...
    bitmap.width = Math.ceil(
      (ink.measureText(source.dataset.country).width + 8) * devicePixelRatio,
    );
    ...
    ink.lineJoin = "round";
    ink.strokeText(
      source.dataset.country,
      bitmap.width / devicePixelRatio / 2,
      bitmap.height / devicePixelRatio / 2,
    );
    ink.fillText(
      source.dataset.country,
      bitmap.width / devicePixelRatio / 2,
  ...
  for (const mode of Object.keys(samples)) {
    for (let phase = 0; phase < 8; phase++) {
      ...
      const screenshot = await page.screenshot({
        clip: { x: 0, y: 0, width: 180, height: 90 },
      });
      samples[mode].push(
        await page.evaluate(async (base64) => {
          ...
          return {
            contrast: energy / mass,
            x: horizontalMoment / mass,
            y: verticalMoment / mass,
          };
  ...
  const range = (values) =>
    Math.max(...values.map((value) => value.contrast)) -
    Math.min(...values.map((value) => value.contrast));
  const motionError = (values) =>
    Math.max(
      ...values
        .slice(1)
        .map((value, index) =>
          Math.hypot(
            value.x - values[index].x - 1 / 8,
            value.y - values[index].y - 1 / 8,
          ),
        ),
    );
  ...
  console.log(
    JSON.stringify({
      ...
      variation: Object.fromEntries(
        Object.entries(samples).map(([mode, values]) => [mode, range(values)]),
      ),
      motionError: Object.fromEntries(
  ...
  assert.ok(
    range(samples.outline) < range(samples.bitmap),
    "outlines have less displayed contrast fluctuation than translated bitmaps",
  );
  assert.ok(
    motionError(samples.outline) < 0.35 / dpr,
    "visible glyphs follow subpixel motion without pixel-row jumps",
  );
}
...
`,
  ...
  for (const [width, dpr] of [
    ...
    await page.waitForFunction(
      () => window.ready && document.querySelector('[data-country="Canada"]'),
    );
    await page.evaluate(() => {
      window.resizes = 0;
    ...
    for (const flat of [false, true]) {
      ...
      for (let frame = 0; frame < 60; frame++) {
        const result = await page.evaluate(
          async ({ frame, flat }) => {
            ...
            const canvas = document
              .querySelector('canvas[role="application"]')
            ...
            return {
              ...
              canvasLabelCopies: window.draws.filter(
                (draw) => draw.name === "Canada",
              ).length,
            };
          },
          { frame, flat },
        ...
        assert.ok(
          result.samePath,
          "glyph outlines stay unchanged across frames",
        );
        ...
        assert.equal(
          result.opacity,
          rotationOpacity,
          "interior label opacity stays constant during rotation",
        );
        assert.equal(
          result.canvasLabelCopies,
          0,
          "labels are not resampled into the globe canvas",
        );
        for (const name of ["Canada", "marker"]) {
          ...
          // Both Canada and the marker are snapped to a quarter device pixel
          // (anti-shimmer), so each anchor can be off by up to an eighth of a
          // device pixel.
          const tolerance = (Math.SQRT2 * 0.125) / dpr + 0.02;
          assert.ok(
            Math.hypot(
    ...
    assert.equal(
      ...
      0,
      "country glyphs never pass through a canvas bitmap",
    );
    const target = await page.evaluate(() =>
      window.draws.find((draw) => draw.name === "marker"),
    );
    ...
    await page.screenshot({
      path: `/private/tmp/hazard-globe-animation-${width}-dpr${dpr}.png`,
    });
  ...
  console.log(
    "PASS: 480 desktop/mobile globe/map frames at DPR 1, 2 and 3; persistent outlines, exact anchors, screenshot contrast and glyph-motion checks, and marker picking.",
  );
} finally {
  await browser?.close();
  await server?.close();
